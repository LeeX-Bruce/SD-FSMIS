...
if __name__ == '__main__':
    ...
    # 创建目录用于保存预测结果和源文件
    os.makedirs(f'result_multiverseg_5-shot/{result_name}', exist_ok=True)
    ...
    for label_val, label_name in labels.items():
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(test_loader):
                ...
                idx_ = np.linspace(0, C_q, 10 + 1).astype('int')
                for sub_chunck in range(10):  # n_part = 3
                    ...
                    target_image = query_image_s[:, [0]]          # (B, 1, H, W)

                    # 创建 5 通道的 target_image，后面 4 通道补 0
                    B, _, H, W = target_image.shape
                    target_image = torch.cat([
                        target_image,                             # ch0: image
                        torch.zeros(B, 4, H, W, device=target_image.device, dtype=target_image.dtype)
                    ], dim=1)
                ...
                # 保存预测结果
                file_name = os.path.join(f"result_multiverseg_5-shot/{result_name}/prediction_{query_id}_{label_name}.nii.gz")
    ...
    with open(f'result_multiverseg_5-shot/{result_name}/{file_name}', 'w') as file:
