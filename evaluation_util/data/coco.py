...
class DatasetCOCO(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        batch = {'rgb_path': os.path.join(self.base_path, query_name),  # TODO delete rgb_path?
                 ...
                 'query_img': query_img,  # [3, img_size, img_size], [-1, 1]
                 'query_mask': query_mask,  # [img_size, img_size], [0, 1]
                 'query_name': query_name,

                 'org_query_imsize': org_qry_imsize,

                 'support_imgs': support_imgs,  # [ n-shot, 3, img_size, img_size], [-1, 1]
                 'support_masks': support_masks,  # [ n-shot, img_size, img_size], [0, 1]
    ...
    def build_class_ids(self):
        ...
        class_ids_val = [self.fold + self.nfolds * v for v in
                         range(nclass_trn)]  # 20 classes: [fold, fold+1, ..., fold+19]
        class_ids_trn = [x for x in range(self.nclass) if x not in class_ids_val]  # 60 classes
